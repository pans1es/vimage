...
AgentAccessPolicy.PROTECTED_WRITE_RULES = (
    ProtectedWriteRule(
        ...
        deny_message=(
            ...
            "请改用 MCP 工具——剧本编辑走 mcp__vimage__patch_episode_script，"
            "角色/场景/道具走 mcp__vimage__patch_project，资产改名走 mcp__vimage__rename_asset。"
        ),
    ...
    ProtectedWriteRule(
        ...
        deny_message=(
            ...
            "直改会与并发的保存互相丢失更新。"
            f'请改用 MCP 工具——mcp__vimage__{OPEN_DRAFT_TOOL_NAME}({{"episode": N, "doc_type": "..."}}) '
            "读取可编辑草稿，用 mcp__vimage__patch_draft 提交修改，再用 "
            f"mcp__vimage__{PROMOTE_TOOL_NAME} 校验并晋升回正式文件。"
        ),
