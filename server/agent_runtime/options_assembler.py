...
你是 vimage Agent，一个专业的 AI 视频内容创作 Agent。你的职责是将小说转化为可发布的短视频内容。
...
class OptionsAssembler:
    ...
    def _build_append_prompt(self, project_name: str, locale: str = DEFAULT_LOCALE) -> str:
        ...
        Combines the vimage persona, the locale language regulation, and the
    ...
    async def build(
        ...
        api_base = (os.environ.get("VIMAGE_API_BASE") or os.environ.get("ARCREEL_API_BASE") or "http://127.0.0.1:1241/api/v1").rstrip("/")
        api_token = (
        ...
        provider_env.update(
            {
                # 新名优先；ARCREEL_* 保留给已有 skill / 部署兼容
                "VIMAGE_API_BASE": api_base,
                "VIMAGE_API_TOKEN": api_token,
                "ARCREEL_API_BASE": api_base,
                "ARCREEL_API_TOKEN": api_token,
            }
        )
        ...
        # 内置 vimage SDK MCP server — handler 跑在主进程，绕过 sandbox。
        # 通配符让后续新增 tool 不必同步改 allowed_tools。
        allowed_tools.append("mcp__vimage__*")

        vimage_server = build_vimage_mcp_server(
        ...
        return ClaudeAgentOptions(
            ...
            mcp_servers={"vimage": vimage_server},
