  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        <AssetSidebar />
        <main
          className="or-well flex-1 overflow-hidden"
      ...
      <button
        ...
        aria-hidden={assistantPanelOpen}
        className={`fixed right-4 top-[calc(var(--chrome-header-h)+0.5rem)] grid h-11 w-11 place-items-center rounded-xl transition-all duration-300 ease-in-out ${UI_LAYERS.workspaceFloating} ${
          assistantPanelOpen
        ...
        style={{
          ...
          boxShadow:
            "inset 0 1px 0 oklch(1 0 0 / 0.2), 0 0 0 1px oklch(0.32 0.07 172), 0 8px 22px -8px var(--color-accent-glow)",
          transitionDelay: assistantPanelOpen ? "0ms" : "200ms",
        }}
        title={t("open_assistant_panel")}
        aria-label={t("open_assistant_panel")}
      >
        <Robot className="h-5 w-5" weight={ICON.weight} />
      </button>
      )}
    </div>
  );
