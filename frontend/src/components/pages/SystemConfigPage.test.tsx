...
describe("SystemConfigPage", () => {
  ...
  it("loads version info when entering the about section", async () => {
    ...
    expect(await screen.findByText("0.10.1")).toBeInTheDocument();
    expect(await screen.findByText(/发布时间/)).toBeInTheDocument();
    expect(screen.queryByText("Release Notes")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /检查更新/ })).not.toBeInTheDocument();
  });

  it("does not fetch system version for the about section display", async () => {
    const getSystemVersion = vi.spyOn(API, "getSystemVersion").mockResolvedValue(
      makeVersionResponse({ latest: null, has_update: false }),
    );

    renderPage("/app/settings?section=about");
    await screen.findByText("0.10.1");
