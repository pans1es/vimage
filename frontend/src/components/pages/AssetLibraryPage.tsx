  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-bg text-text">
        ...
        >
          {TABS.map(({ type, icon: Icon }) => {
            ...
            const cls = active
              ...
              : "border-hairline-soft bg-field-muted text-text-2 hover:border-hairline hover:text-text";
            return (
              <button
                key={type}
                ref={(el) => {
              ...
              >
                ...
                <span
                  className={`rounded-full px-1.5 py-0.5 font-mono text-[10px] font-semibold tabular-nums ${
                    active ? "bg-accent-soft text-accent-2" : "bg-field text-text-4"
                  }`}
                >
                  {count}
                </span>
      ...
      <main className="relative mx-auto w-full max-w-6xl flex-1 px-6 py-8">
        ...
        >
        {assets.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-hairline bg-field-muted py-24 text-center">
